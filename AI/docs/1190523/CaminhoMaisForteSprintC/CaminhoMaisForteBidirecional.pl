...
dfsMaisForte2(Act, Dest, LA, Cam, Forca) :-
        ...
        ( ligacao(NAct, NX, F1, F2, FR1, FR2); ligacao(NX, NAct, F2, F1, FR2, FR1) ),       % se existir ligacao NAct->NX ou NX -> NAct, prossegue
        ...
        Forca is (FX + F1 + F2 + FR1 + FR2).                                        % Forca passara a ter o valor de FX que vem de tras, somado com as forcas de ligacao F1 e F2
