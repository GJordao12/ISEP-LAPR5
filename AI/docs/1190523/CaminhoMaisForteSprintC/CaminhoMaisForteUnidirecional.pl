...
dfsMaisForteUni2(Act, Dest, LA, Cam, Forca) :-
        ...
        ( ligacao(NAct, NX, F1, _, FR1, _); ligacao(NX, NAct, _, F1, _, FR1) ),                 % se existir ligacao NAct->NX ou NX -> NAct, prossegue
        ...
        Forca is (FX + F1 + FR1).                                                     % Forca passara a ter o valor de FX que vem de tras, somado com a forcas de ligacao F1
