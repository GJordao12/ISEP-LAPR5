...
bestfs1_mc_one(Orig,Dest,Cam,Custo):-
                bestfs12_mc_one(Dest,[[Orig]],Cam,Custo).

bestfs12_mc_one(Dest,[[Dest|T]|_],Cam,Custo):-
                reverse([Dest|T],Cam), calcula_custo_mc(Cam,Custo), write('Caminho='),write(Cam),nl.

bestfs12_mc_one(Dest,[[Dest|_]|_],_,_):-
                !.

bestfs12_mc_one(Dest,LLA,Cam,Custo):-
                member1(LA,LLA,LLA1),LA=[Act|_],
                ((Act==Dest,!,bestfs12_mc_one(Dest,[LA|LLA1],Cam,Custo)) ; (findall((CX,[X|LA]),
                (no(NAct,Act,_),no(NX,X,_),(ligacao(NAct,NX,FL,_,FR,_);ligacao(NX,NAct,_,FL,_,FR)),CX is FL+FR,
                \+member(X,LA)),Novos),
                Novos\==[],!,
                sort(0,@>=,Novos,NovosOrd),
                ...
                append(NovosOrd1,LLA1,LLA2),
                %write('LLA2='),write(LLA2),nl,
                bestfs12_mc_one(Dest,LLA2,Cam,Custo))).
